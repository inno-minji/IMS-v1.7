function chkIns(){
	with (document.frm)
	{
		...
		if (sDealMaeipCode.selectedIndex == 0)
		{
			alert("매입상태를 선택하세요.");
			sDealMaeipCode.focus();
			return  ;
		}
...
//			alert(sComName.value + sComKey.value +"++++++++sComName.value + sComKey.value ");
		...
		if (sDealMaeipDate.value == "")
		{
			alert("매입일을 입력하세요.");
			sDealMaeipDate.focus();
			return false;
			
		}
		
		if (sDealMaeipTotalMoney.value == "")
		{
			alert("발주총액을 입력하세요.");
			sDealMaeipTotalMoney.focus();
			return false;
		}
		
		if (sDealMaeipRealMoney.value == "")
		{
			alert("실매입액을 입력하세요.");
			sDealMaeipRealMoney.focus();
			return false;
		}
...
//				);
		...
		innerStr+="<input type='hidden' id='maeipItemKey' 	name='maeipItemKey' 		value='0'/>";
...
function comPop(){ 
		...
		var sName = "AddrAdd";
		if(!win||win.closed){
			win = window.open(sUrl,sName,"width=520, height=500, scrollbars=yes, resizeable=no, left=150, top=150");
			win.opener.self;
		}else{
			win.location = sUrl;
			win.focus();
