...
<script type="text/javascript">
	...
	function sel_assetTwo(){	
		var sUrl = "/mes/maintance/kw_maintance_box_lf.do?searchType=Y&mMaintanceKey="+$("#mMaintanceKey").val();
		window.open(sUrl, "AddrAdd", "width=1400, height=650, status=no, toolbar=no, resizable=yes, menubar=no, location=no, scrollbars=yes");
		window.focus();
	}
	
	function setMaintanceReturnPop(obj){  
		$.ajax({
				type		: "post"
			,	dataType	: "json"
			,	url			: "/mes/maintance/kw_getMaintancenfoList.do"
			,	data		: {
				mMaintanceKey : obj.mMaintanceKey
				}
			,	success		: function(msg){
				var eInfoList  = msg.result.dataList;
					for(var i = 0; i < eInfoList.length; i++){
						setMainPop(eInfoList[i]);
					}
				}
	...
	var row_Indextt = 0;
	function setMainPop(obj){
		
	    var isDuplicate = false; // 중복 여부를 체크할 변수
		var mMaintanceKeyArr = document.getElementsByName("eRowMaintanceKey").length;
		if(mMaintanceKeyArr == 0){
			var tbody = document.getElementById("lineRowTwo");
		    tbody.innerHTML = "";  
		}else{
			 var existingKeys = []; // 기존에 존재하는 mMaintanceKey 값들을 담을 배열
			    
			    // 기존에 존재하는 mMaintanceKey 값을 배열에 담기
			    var existingElements = document.getElementsByName("eRowMaintanceKey");
			    for (var i = 0; i < existingElements.length; i++) {
			        existingKeys.push(existingElements[i].value);
			    }
			    
			    // obj.mMaintanceKey 값이 existingKeys 배열에 없는 경우만 추가
			    for (var i = 0; i < existingElements.length; i++) {
			        var currentKey = existingElements[i].value;
			        
			        // obj.mMaintanceKey와 중복되는지 체크
			        if (currentKey === obj.mMaintanceKey) {
			            isDuplicate = true;
			            break; // 중복이면 더 이상 체크할 필요 없음
			        }
			    }
	  ...
	  if (!isDuplicate) {
			...
			innerStr += "			<a class='del' onclick=\"delRow(this);\">X</a>";
			innerStr += "		</td>";
			innerStr += "		<td>" +obj.mMaintanceNumber;
			innerStr += "			<input type='text' id='eRowMaintanceKey_"+row_Indextt+"' name='eRowMaintanceKey' value='"+obj.mMaintanceKey+"'/>";
			innerStr += "		</td>";
			innerStr += "		<td>" +obj.eAssetTypeName;
			innerStr += "		</td>";		
			// 자산유형
			innerStr += "		<td>" +obj.mMaintanceTypeName;
			innerStr += "		</td>";
			innerStr += "		<td>" +obj.mMaintanceCateName;
			innerStr += "		</td>";
			innerStr += "		<td>"+obj.mRequestDate;
			innerStr += "		</td>";	
			innerStr += "		<td>"+obj.mMaintanceContent;
			innerStr += "		</td>";	
			innerStr += "	</tr>";
		...
		<table>
			<caption style="text-align: left; margin-bottom:10px;">
				...
				<tr>
					...
					<th style="width: 12%;">자산유형</th>
					<th style="width: 12%;">처리유형</th>
					<th style="width: 12%;">요청기간</th>
