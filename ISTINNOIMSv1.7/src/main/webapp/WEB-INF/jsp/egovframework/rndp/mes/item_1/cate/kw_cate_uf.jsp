...
function update_go(){
	if(chkIns()){
		if(confirm("저장하시겠습니까?")){
			$("#mloader").show();
			document.frm.action = "/mes/item/cate/kw_cate_u.do";
			document.frm.submit();
		}
	}
}
...
<form id="frm" name="frm" method="post" action="/mes/item/cate/kw_cate_lf.do">
	...
	<input type="hidden" id="sItemCateUKey" name="sItemCateUKey" value="${cateInfo.sItemCateUKey}" />
  		...
  		<table>
	  		<tbody>
				...
				<tr>
	      			<th>*품목분류명</th>
	      			<td>
	      				<input style="width: 90%;" type="text" id="sItemCateName" name="sItemCateName" value="${cateInfo.sItemCateName}" />
	      			</td>
	   		 	</tr>
				<tr>
					<th>*코드번호</th>
					<td>
						<c:if test="${empty cateInfo}">
							<input style="width: 90%; text-transform: uppercase;" type="text" id="sItemCodeHead" name="sItemCodeHead" value="${cateInfo.sItemCodeHead}"  maxlength="2" />
						</c:if>
	   					<c:if test="${!empty cateInfo}">
				...
				<tr>
					<th>*LOGO CODE</th>
					<td>
						<input style="width: 90%; text-transform: uppercase;" type="text" id="logoCode" name="logoCode" value="${cateInfo.logoCode}" maxlength="2" />
					</td>
				</tr>
				<tr>
	      			<th>비고</th>
	      			<td>
	      				<input style="width: 90%;" type="text" id="sItemCateEtc" name="sItemCateEtc" value="${cateInfo.sItemCateEtc}" />
	      			</td>
	    		</tr>
	  		</tbody>
		</table>
