...
function go_insert(){
	//$('#mloader').show();
	document.frm.action = "/jsp/homeEdu/kw_edu_if.do";
	document.frm.submit();
}
...
function viewBom(key){
	...
	document.frm.action = "/jsp/homeEdu/kw_edu_vf.do";
	document.frm.submit();
}
...
<!--컨텐츠 시작-->
<form id="frm" name="frm" method="post" action="/mes/bom/kw_bom_lf.do">
	...
	<table  class="board_list">
		...
		<TBODY>
			<c:forEach var="item" items="${eduList}">
				<tr>
					<td class="subject">${item.kEduSubject}</td>
					<td>${item.kEduStartdate}~${item.kEduEnddate}</td>
					<td>${item.kEduActiveflag}</td>
				</tr>
			</c:forEach>
