...
function fileExtCheck(obj) {
    ...
    var officeExtensions = ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'hwp', 'hwpx'];
    var officeMimeTypes = [
    ...
    var uploadMessage = fileCheckGubun == "Y" 
        ? 'PDF, 이미지 파일(.jpg, .jpeg, .png, .gif, .bmp, .webp) \n및 오피스 문서(.doc, .docx, .xls, .xlsx, .ppt, .pptx, .hwp, .hwpx)만 업로드 가능합니다.' 
    ...
    // MIME 타입 확인   브라우저 체크 -> 서버체크로 바꿈
    if (allowedMimeTypes.indexOf(file.type) === -1) {
//        alert('허용되지 않은 파일 형식입니다.');
//        resetFileInput(obj);
//        return;
		$("#searchWord").val("reqCheckMIME");
    }
...
<form id="frm" name="frm" method="post"  enctype="multipart/form-data"  action="/mes/asset/popup/mesPDFAdd_insert.do" class="popup_wrap">
	...
	<input type="hidden" id='searchWord' name="searchWord" value="${mesAssetVO.searchWord}"/>  <%--  파일 업로드 오류로 임시로 아무거나 사용함 (MIME타입 체크)  --%>
	<input type="hidden" id='searchType' name="searchType" value="${mesAssetVO.searchType}"/>  <%--  파일 업로드 오류로 임시로 아무거나 사용함 (MIME타입 체크)  --%>
