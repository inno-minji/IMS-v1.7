...
<script type="text/javascript">
	function move_go(){
		if (document.searchform.root2.value == ""){
			alert("메뉴를 선택하세요.");
			return;
		}
...
<div id="contents">
  ...
  <form name="searchform" id="adminmenuVO" method="post" action="<c:url value='/admin/adminMenuOrder.do'/>">
		...
		<td>
		  <select name="root2">
			...
			<c:forEach var="item" items="${result}" varStatus="status">
				<option value="${item.root}">${item.name}</option>	
			</c:forEach>			
		  </select>
		</td>
      </tr>
 ...
 <div class="tbl_btn_right">
	<ul>
		<li>
			<a href="javascript:move_go();">확인</a>
		</li>
		<li>
			<a href="javascript:move_reset();">수정</a>
		</li>
		<li>
