...
<script type="text/javascript">
	...
	function fileExtCheck(obj, cnt){   
		...
		if(cnt == 1){
			...
			prefixList = ['mp4','MP4','gif','GIF', 'jpeg', 'JPEG', 'bmp', 'BMP', 'jpg', 'JPG', 'png', 'PNG', 'TIFF', 'tiff', 'TIF', 'tif'];
		}
		var fielExt = getFileExtension(obj);
	   	...
	   	document.getElementById("filename_"+cnt).select();
				...
				<table cellspacing="0" cellpadding="0" width="100%" border="0" class="board_view">
					...
					<c:forEach items="${fileList}" var="fileList" varStatus="i">	
						<tr id="${fileList.fileKey}">
							...
							<td class="td_view" colspan="3">	
								...
								<c:if test="${i.index != 0 }">
									<a href="#none;" onClick="fileKeyDelete('${fileList.fileKey}', '${i.index}');"><img src="./images_in/btn/btn_del_s.gif" align="absmiddle"> </a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
						...
						<tr>
							<th class="line_right" scope="row">첨부파일${j.index+1}</th>
						 	<td class="td_view" colspan="3">
							 	...
							 	<input type='file' name='filename_${fileKeyNm}_${j.index}' id='filename_${j.index}' maxLength='100' size='45' class='' onChange="fileChk(this,'${j.index}','filename_${fileKeyNm}_${j.index }');"/>(용량 : ${boardmng.fileSize } MB)
					...
					<tr>
						<Td colspan="4" style="text-align: center; height: 50px;">
							<b>파일 변경시에는 변경하고싶은 첨부파일의 번호에서 첨부문서를 다시 선택하면 됩니다.</b>  
						</Td>
					</tr>
	...
	<tr>
		<td>
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				<tr height="35">
					<td align="right">						
						<input type="image" src="/images/images_btn/v_ok.gif" onClick="javascript:fn_update();" /> <!--확인 btn-->
						<a href="javascript:insert_reset();"><img src="/images/images_btn/v_cancle.gif" border="0" /></a> <!--취소 btn-->
						<a href="<c:url value='/webMenu.do'/>?key=${key}&pageGb=r&groupKey=${groupKey}"><img src="/images/images_btn/v_list.gif" border="0" /></a> <!--목록 btn-->
					</td>
				</tr>
			</table>
		</td>
